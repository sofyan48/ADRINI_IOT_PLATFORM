...
def parser(json, command=None):
    ...
    for i in json:
        ...
        for parameters_key in endpoint[i]:
            if parameters_key == "depend":
                depend = endpoint[i][parameters_key]
                if depend.split(":")[0] == "func" and len(depend.split(":")) == 3:
                    funct_default = {
                        'type': depend.split(":")[0],
                        'name': depend.split(":")[2],
                        'moduls': depend.split(":")[1],
                        'parameters': None
                    }
                else:
                    # print(type(len(depend.split(":"))))
                    params = list()
                    key_params=0
                    while key_params <= len(depend.split(":")) - 1:
                        if key_params >= 3 :
                            params.append(depend.split(":")[key_params])
                        key_params= key_params+1
                    # print(params)
                    funct_default = {
                        'type': depend.split(":")[0],
                        'moduls': depend.split(":")[1],
            ...
            if parameters_key == "moduls":
                moduls_func = endpoint[i][parameters_key]
                # print("Data MODULS: ", moduls_func)
